...
namespace AsyncSocket
{
    ...
    {        
        ...
        public int nRoomNumber = -1; //!< 초기값 -1 대기실이다
        public E_CLIENTSTATE eState = E_CLIENTSTATE.E_CLIENTSTATE_NONE;
        ...
        public bool Send(byte[] buffer)
        {
            ...
            {
                Console.WriteLine("SendError!{0}", e.ToString());
                AsyncSocketErrorEventArgs eev = new AsyncSocketErrorEventArgs(this.id, e);
    ...
    public class AsyncSocketServer : AsyncSocketClass
    {
        ...
        public AsyncSocketServer(string iserverIP, int port)
        {
            serverIP = iserverIP;
            this.port = port;            
        }
        ...
        public void Listen()
        {
            try
            {
                ...
                listener.Bind(new IPEndPoint(IPAddress.Parse(serverIP), this.port));
